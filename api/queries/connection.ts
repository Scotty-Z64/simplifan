...
function seedData(db: Database.Database) {
  ...
  // Seed services
  db.prepare(`INSERT INTO vendor_services (vendorId, name, description, price, category) VALUES (?, ?, ?, ?, ?)`)
    .run(1, 'Buffet Catering', 'Full buffet service with setup', '350.00', 'Catering');
  db.prepare(`INSERT INTO vendor_services (vendorId, name, description, price, category) VALUES (?, ?, ?, ?, ?)`)
    .run(2, 'DJ + Sound System', 'Full PA system with DJ', '5500.00', 'Music');
  db.prepare(`INSERT INTO vendor_services (vendorId, name, description, price, category) VALUES (?, ?, ?, ?, ?)`)
    .run(3, 'Photography Package', 'Full day coverage', '8000.00', 'Photography');

  // Seed images
  db.prepare(`INSERT INTO vendor_images (vendorId, url, caption) VALUES (?, ?, ?)`)
    .run(1, 'https://picsum.photos/400/300?random=1', 'Catering');
  db.prepare(`INSERT INTO vendor_images (vendorId, url, caption) VALUES (?, ?, ?)`)
    .run(2, 'https://picsum.photos/400/300?random=2', 'DJ');
  db.prepare(`INSERT INTO vendor_images (vendorId, url, caption) VALUES (?, ?, ?)`)
    .run(3, 'https://picsum.photos/400/300?random=3', 'Photo');

  // Seed events
  db.prepare(`INSERT INTO events (clientId, clientName, clientPhone, eventType, eventDate, eventTime, guestCount, province, city, area, venue, budget, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(1, 'Thabo Mokoena', '0823456789', 'Wedding', '2026-08-15', '14:00', 120, 'Gauteng', 'Johannesburg', 'Soweto', 'Community Hall', '85000.00', 'planning', 'Traditional wedding');
  db.prepare(`INSERT INTO events (clientId, clientName, clientPhone, eventType, eventDate, eventTime, guestCount, province, city, area, venue, budget, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(2, 'Lerato Khumalo', '0712345678', 'Birthday', '2026-07-22', '18:00', 50, 'Gauteng', 'Sandton', 'Morningside', 'Private Residence', '25000.00', 'planning', '30th birthday');

  // Seed bookings
  db.prepare(`INSERT INTO bookings (eventId, clientId, clientName, clientPhone, vendorId, vendorName, eventType, eventDate, amount, depositAmount, platformFee, status, clientConfirmed, vendorConfirmed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(1, 1, 'Thabo Mokoena', '0823456789', 1, 'Royal Events SA', 'Wedding', '2026-08-15', '35000.00', '10000.00', '1750.00', 'pending', 0, 0);
  db.prepare(`INSERT INTO bookings (eventId, clientId, clientName, clientPhone, vendorId, vendorName, eventType, eventDate, amount, depositAmount, platformFee, status, clientConfirmed, vendorConfirmed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(1, 1, 'Thabo Mokoena', '0823456789', 2, 'DJ Maphorisa Sounds', 'Wedding', '2026-08-15', '5500.00', '2000.00', '275.00', 'confirmed', 1, 1);

  // Seed reviews
  db.prepare(`INSERT INTO reviews (vendorId, clientId, clientName, bookingId, rating, comment, eventType, verifiedBooking) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(1, 1, 'Thabo Mokoena', 1, 5, 'Amazing food! Our guests are still talking about the braai platters.', 'Wedding', 1);
  db.prepare(`INSERT INTO reviews (vendorId, clientId, clientName, bookingId, rating, comment, eventType, verifiedBooking) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(2, 2, 'Lerato Khumalo', 2, 5, 'DJ Maphorisa kept the dance floor packed all night!', 'Wedding', 1);

  // Seed conversations
  db.prepare(`INSERT INTO conversations (clientId, vendorId, lastMessage, clientUnread, vendorUnread) VALUES (?, ?, ?, ?, ?)`)
    .run(1, 1, 'Hi, can you accommodate dietary requirements?', 0, 1);
  db.prepare(`INSERT INTO conversations (clientId, vendorId, lastMessage, clientUnread, vendorUnread) VALUES (?, ?, ?, ?, ?)`)
    .run(1, 2, 'What time should you arrive to set up?', 1, 0);

  // Seed messages
  db.prepare(`INSERT INTO messages (conversationId, senderType, content, read) VALUES (?, ?, ?, ?)`)
    .run(1, 'client', 'Hi, I am planning my wedding and interested in your catering services.', 1);
  db.prepare(`INSERT INTO messages (conversationId, senderType, content, read) VALUES (?, ?, ?, ?)`)
    .run(1, 'vendor', 'Congratulations! We would love to be part of your special day.', 1);
  db.prepare(`INSERT INTO messages (conversationId, senderType, content, read) VALUES (?, ?, ?, ?)`)
    .run(1, 'client', 'Around 120 guests. Can you accommodate dietary requirements?', 0);
